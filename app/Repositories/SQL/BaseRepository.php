...
abstract class BaseRepository implements BaseContract
{
    ...
    public function applyRelations($query, $relations)
    {
        if (!empty($relations)) {
            ...
            $normalRelations = array_diff($relations, $filteredRelations);
            if (!empty($filteredRelations)) {
                foreach ($filteredRelations as $relation) {
                    $relationName = explode(':', $relation)[0];
                    $relationColumns = str_replace("-", ",", explode(':', $relation)[1]);
                    $query = $query->with("$relationName:$relationColumns");
                }
            } else {
            ...
            if (!empty($normalRelations)) {
                $query = $query->with($normalRelations);
            }
        }
        return $query;
    }
    ...
    public function getQueryResult($query, array $data = []): mixed
    {
        ...
        $order = $data['order'] ?? [];
        $columns = !empty($data['columns']) ? explode(',', $data['columns']) : null;
        ...
        if (isset($withCount)) {
            $withCount = is_array($withCount) ? $withCount : [$withCount];
        ...
        if (!empty($groupBy)) {
            if (!is_null($columns)) {
                return $query->select($columns)->get()->groupBy($groupBy);
            }
        ...
        if ($customizePaginationURI) {
            if (!is_null($columns)) {
                $query = $query->select($columns)->paginate($limit);
            } else {
        ...
        if ($limit) {
            if (!is_null($columns)) {
                return $query->take($limit)->select($columns)->get();
            }
